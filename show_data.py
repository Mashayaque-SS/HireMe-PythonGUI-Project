...
class data_display:
    def __init__(self,root):
        ...
        # Home Button **************************
        b2=Button(self.deframe,text="Home",command=self.hm,font=("times new roman",20,"bold"),fg="blue",bg="white",activebackground="white",activeforeground="blue",bd=6)
        b2.place(x=1210,y=5)
        ...
        wee=ttk.Entry(self.deframe,textvariable=self.work_ent,font=("times new roman",15,"bold"))
        ...
        #--csc
        csc=Label(self.deframe,text="Country :",font=("times new roman",15,"bold"),fg="white",bg="#1877f2",bd=3).place(x=630,y=285)
        ...
        #--Hobby
        intrests=Label(self.deframe,text="Hobby :",font=("times new roman",15,"bold"),fg="white",bg="#1877f2",bd=3).place(x=940,y=110)
        ...
        b2=Button(btnframe,text="Update",command=self.update,fg="white",bg="blue",activebackground="blue",activeforeground="white",bd=4,
                    font=("times new roman",18,"bold"))
        b2.place(x=170,y=10,width=145,height=45)

        b3=Button(btnframe,text="Delete",command=self.delete,fg="white",bg="blue",activebackground="blue",activeforeground="white",bd=4,
                    font=("times new roman",18,"bold"))
        b3.place(x=10,y=60,width=145,height=45)

        b4=Button(btnframe,text="Clear",command=self.clear,fg="white",bg="blue",activebackground="blue",activeforeground="white",bd=4,
                    font=("times new roman",18,"bold"))
        b4.place(x=170,y=60,width=145,height=45)

        b5=Button(btnframe,text="Exit",command=self.exit,fg="white",bg="blue",activebackground="blue",activeforeground="white",bd=4,
                    font=("times new roman",18,"bold"))
        ...
        self.info_table=ttk.Treeview(dataframe,column=("name","gender","age",
                                    "qualification","post","work","languages","achievements","hobby","csc","email"
        ...
        self.info_table.heading("work",text="Work Experience")        
        ...
        self.info_table.heading("csc",text="Contry")
        ...
        self.info_table.column("work",width=100)
        ...
        self.info_table.bind("<ButtonRelease-1>",self.ftf)
        self.fetchdata()
    ...
    def update(self):         
        conn=mysql.connector.connect(host="localhost",username="root",password="mysql",database="hireme")
        cur=conn.cursor()
        cur.execute("update data set Name=%s, Gender=%s, Age=%s, Qualification=%s, Post=%s, WorkExperience=%s, Languages=%s, Achievements=%s, Hobby=%s, Country=%s where Email=%s",(
                        self.name_ent.get(),
                        self.gen_ent.get(),
                        self.age_ent.get(),
                        self.quali_ent.get(),
    ...
    def ftf(self,show=""):
        cur_row=self.info_table.focus()
        content=self.info_table.item(cur_row)
        row=content["values"]

        self.name_ent.set(row[0])
        self.gen_ent.set(row[1])
        self.age_ent.set(row[2])
        self.quali_ent.set(row[3])
        self.post_ent.set(row[4])   
        self.work_ent.set(row[5])                                                                                   
        self.lang_ent.set(row[6])
        self.ach_ent.set(row[7])
        self.hobby_ent.set(row[8])
        self.csc_ent.set(row[9])
        self.email_ent.set(row[10])
    ...
    def exit(self):
        box=messagebox.askyesno("HireMe","Do you want to exit ?")
        if box>0:
            self.root.destroy()
            return
