...
class wipro:
    def __init__(self,root):
        self.root=root
        self.root.title("Wipro PVT LMT")
        self.root.geometry("1360x768")
        self.root.state('zoomed')

        # image and info
        img=Image.open(r"E:\Mashayaque\First Year\Sem 2\Project\Recruitment System\wip.jpg")
        img=img.resize((1360,768))
        self.photoimg=ImageTk.PhotoImage(img)
        lblimg=Label(self.root,image=self.photoimg,bd=4).place(x=0,y=0,width=1360,height=768)
        lb1=Label(self.root,text="Wipro is a Multinational Company i.e. An MNC, which makes it one among the top companies",font=("times new roman",15),bg="white",fg="black")
        lb1.place(x=5,y=5)
        lb2=Label(self.root,text="Wipro founder is Azim Hashim Premji",font=("times new roman",15),bg="white",fg="black")
        lb2.place(x=5,y=35)
